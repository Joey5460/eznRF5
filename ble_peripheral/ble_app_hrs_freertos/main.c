...
#include "tasks/states/dev_states.h"
#include "libraries/bsp/bsp_mpu.h"
#include "ble_services/ble_syncs/ble_syncs.h"
#include "ble_services/ble_imus/ble_imus.h"
...
static void sync_write_callback(ble_sync_t * p_sync, uint8_t cmd)
{
    switch(cmd){
        case 0x01:
			NRF_LOG_INFO("sync 0x01\r\n");
            dev_state_data_set(EVT_SYNC);
            break;
        case 0x02:
            log_clear(); 
            break;
...
static void test_write_callback(ble_sync_t * p_sync, uint8_t * cmd)
{
    uint16_t  c = cmd[0] + (cmd[1]<<8);
    switch(c){
        case 0x00:
 //           mpu_tst_on = 0;
			NRF_LOG_INFO("CMD 0x00");
            break;
        case 0x01:
 //           mpu_tst_on = 1;
            break;
...
static void setting_write_callback(ble_sync_t * p_sync, uint8_t* data )
{
    if (0x01 == data[0]){
        uint32_t utc_s = data[4];
...
#define FIRMWARE_REV	"0.0.1"
	...
	ble_sync_init_t sync_init; 
    memset(&sync_init, 0, sizeof(sync_init));

    sync_init.sync_write_handler = sync_write_callback;
    ...
    // Initialize Inertial Measurement Unit Service.
    ble_imu_init_t imu_init;
    memset(&imu_init, 0, sizeof(imu_init));

    //imu_init.imu_write_handler = imu_write_callback;
    //imu_init.led_write_handler = led_write_callback;
    //imu_init.rate_write_handler = rate_write_callback;
...
static volatile bool ble_tx_done = true;
...
void send_packet(char packet_type, void *data)
{
    ...
    if (packet_type == PACKET_TYPE_ACCEL || packet_type == PACKET_TYPE_GYRO) {
        ...
        buf[17] = (char)ldata[3];
        */
        buf[2-2] = (char)(ldata[0] >> 24);
        buf[3-2] = (char)(ldata[0] >> 16);
        buf[4-2] = (char)(ldata[0] >> 8);
        buf[5-2] = (char)ldata[0];
        buf[6-2] = (char)(ldata[1] >> 24);
        buf[7-2] = (char)(ldata[1] >> 16);
        buf[8-2] = (char)(ldata[1] >> 8);
        buf[9-2] = (char)ldata[1];
        buf[10-2] = (char)(ldata[2] >> 24);
        buf[11-2] = (char)(ldata[2] >> 16);
        buf[12-2] = (char)(ldata[2] >> 8);
        buf[13-2] = (char)ldata[2];
        buf[14-2] = (char)(ldata[3] >> 24);
        buf[15-2] = (char)(ldata[3] >> 16);
        buf[16-2] = (char)(ldata[3] >> 8);
        buf[17-2] = (char)ldata[3];
        //length = 18;
    }
    //uart_handler((uint8_t*) buf, length);
    //cdcSendDataWaitTilDone((BYTE*)buf, length, CDC0_INTFNUM, 100);

	uint32_t err_code = 0; 
	ble_tx_done = false;
	err_code = ble_quat_update(&m_imu, (uint8_t*)buf);
...
static void st_tx(uint8_t cur_st, uint8_t* data)
{
	if (ST_SYNC == cur_st){
		...
		send_packet(PACKET_TYPE_QUAT, data);
...
int main(void)
{
    ...
    if (pdPASS != xTaskCreate(states_thread, "STAT", 256, NULL, 1, &m_states_task))
    {
        APP_ERROR_HANDLER(NRF_ERROR_NO_MEM);
    }
